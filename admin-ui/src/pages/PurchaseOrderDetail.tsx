...
export default function PurchaseOrderDetail() {
  ...
  React.useEffect(() => {
    ...
    (data?.purchaseOrderReceiptProgress || []).forEach((p: any) =>
      progress.set(p.productVariantId, {
    ...
    setRows(
      items.map((it: any) => ({
        ...
        toReceive: it.quantity,
        ordered: progress.get(it.productVariantId)?.ordered ?? it.quantity,
        received: progress.get(it.productVariantId)?.received ?? 0,
      })),
    );
  }, [data]);
  ...
  const setRow = (
    idx: number,
    patch: Partial<{ selected: boolean; toReceive: number }>,
  ) => {
  ...
  const submitReceive = async () => {
    const items = rows
      .filter((r) => r.selected && r.variantId && r.toReceive > 0)
      .map((r) => ({ productVariantId: r.variantId, quantity: r.toReceive }));
    if (!items.length)
      return notify('Select at least one item with quantity', 'warning');
    if (!storeId || !receivedById || !confirmedById)
      return notify('Select store and users', 'warning');
    try {
      await receive({
        variables: {
          input: {
              ...
              {loading ? (
                ...
                <Stack>
                  <Typography color="text.secondary">
                    Supplier: {po.supplier?.name || po.supplierId}
                  </Typography>
                  <Typography color="text.secondary">
                    Status: {po.status} • Phase: {po.phase || '—'}
                  </Typography>
                  <Typography color="text.secondary">
                  ...
                  <Typography>
                    Total:{' '}
                    {po.totalAmount == null ? '—' : formatMoney(po.totalAmount)}
                  </Typography>
                </Stack>
              ) : null}
            </CardContent>
            ...
            <TableBody>
              {(po?.items ?? []).map((it: any, idx: number) => (
                <TableRow key={it.productVariantId} hover>
                  <TableCell padding="checkbox">
                    <Checkbox
                      checked={rows[idx]?.selected || false}
                      onChange={(e) =>
                        setRow(idx, { selected: e.target.checked })
                      }
                    />
                  </TableCell>
                  <TableCell>
