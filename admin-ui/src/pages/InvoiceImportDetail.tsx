import {
  useInvoiceImportQuery,
  useStoresForImportDetailQuery,
  useApproveInvoiceImportMutation,
  useReprocessInvoiceImportMutation,
  useUpdateInvoiceImportMutation,
...
export default function InvoiceImportDetail() {
  ...
  const skip = !id;
  const { data, loading, error, refetch } = useInvoiceImportQuery({
    variables: { id: id as string },
    skip,
    fetchPolicy: 'cache-and-network' as any,
  });
  const { data: storesData } = useStoresForImportDetailQuery({
    fetchPolicy: 'cache-first' as any,
  });
  const [approve, { loading: approving, error: approveError }] =
    useApproveInvoiceImportMutation();
  const [reprocess, { loading: reprocessing, error: reprocessError }] =
    useReprocessInvoiceImportMutation();
  ...
  const [updateImport, { loading: updating }] =
    useUpdateInvoiceImportMutation();
  const [searchPOs, searchPO] = usePurchaseOrdersSearchLazyQuery();
  ...
  React.useEffect(() => {
    const ls = Array.isArray(item?.parsed?.lines)
      ? (item?.parsed?.lines as any[])
      : [];
  ...
  const lines = Array.isArray(item?.parsed?.lines)
    ? (item?.parsed?.lines as any[])
    : [];
  ...
  const computedTotal = lines.reduce((s: number, ln: any) => {
    ...
    const unit =
      ln.unitPrice ?? (ln.lineTotal != null ? Number(ln.lineTotal) / q : 0);
  ...
  const isAlreadyApproved = statusUpper === 'COMPLETED' || statusUpper === 'APPROVED';
  const currentInvoiceNo = String(
    (item?.parsed as any)?.invoiceNumber || invoiceNumber || '',
  ).trim();
  const hasSupplier = Boolean((supplierName || '').trim());
  const hasStore = Boolean((storeId || '').trim());
  const hasInvoiceNo = Boolean(currentInvoiceNo);
  const canApproveNow =
    !isAlreadyApproved && hasSupplier && hasStore && hasInvoiceNo;
  ...
  return (
    <Stack spacing={2}>
      <Stack direction="row" alignItems="center" spacing={1}>
        ...
        <Button size="small" onClick={() => navigate('/invoice-imports')}>
          Back
        </Button>
      </Stack>
      <Card>
          ...
          <Stack spacing={1}>
            ...
            >
              ...
              <Button
                ...
                onClick={async () => {
                  await updateImport({
                    variables: { input: { id, url: localUrl } },
                  });
                  await refetch();
                  notify('Invoice URL updated', 'success');
                }}
              >
                Save URL
              </Button>
              ...
              {edit && (
                <>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setDraftLines(
                        Array.isArray(item?.parsed?.lines)
                          ? (item?.parsed?.lines as any[]).map((l: any) => ({
                              ...l,
                            }))
                          : [],
                      )
                    ...
                    onClick={() =>
                      setDraftLines((arr) =>
                        arr.map((l: any) => {
                          ...
                          const unit =
                            l.discountedUnitPrice ?? l.unitPrice ?? 0;
                          const lineTotal =
                            l.lineTotal == null || isNaN(l.lineTotal)
                              ? +(q * unit).toFixed(2)
                              : l.lineTotal;
                          const discountedUnitPrice =
                            l.discountedUnitPrice == null &&
                            l.discountPct != null &&
                            l.unitPrice != null
                              ? +(
                                  l.unitPrice *
            ...
            <Stack direction="row" spacing={1} alignItems="center">
              ...
              <FormControlLabel
                control={
                  <Switch
                    checked={useParsedTotal}
                    onChange={(e) => setUseParsedTotal(e.target.checked)}
                  />
                }
                label="Use Parsed Total"
              />
              <Button
                size="small"
                variant="outlined"
              ...
              <Button
                variant="outlined"
                disabled={reprocessing || !canManage}
                onClick={async () => {
                  await reprocess({ variables: { id } });
                  notify('Reprocessing started', 'info');
                  await refetch();
                }}
              >
              ...
              <Button
                variant="contained"
                disabled={
                  !item.url || approving || !canManage || !canApproveNow
                }
                title={
                  !canApproveNow
                    ? isAlreadyApproved
                      ? 'Import already approved'
                      : 'Provide Supplier, Store, and Invoice # before approval'
                    : ''
                }
                onClick={() => {
                  if (isAlreadyApproved) {
                    notify('This import has already been approved', 'info');
                    return;
                  }
                  const missing: string[] = [];
                  if (!hasSupplier) missing.push('Supplier');
                  if (!hasStore) missing.push('Store');
                  if (!hasInvoiceNo) missing.push('Invoice #');
                  if (missing.length) {
                    notify(`Please provide: ${missing.join(', ')}`, 'warning');
                    return;
                  }
                  setOpen(true);
                }}
            ...
            {Math.abs(diff) > 0.5 && (
              <Alert severity="warning">
                Invoice totals differ by ₦{fmt(diff)}. Toggle "Use Parsed Total"
                if needed.
              </Alert>
            )}
            {error && <Alert severity="error">{String(error.message)}</Alert>}
            {approveError && (
              <Alert severity="error">{String(approveError.message)}</Alert>
            )}
            {reprocessError && (
              <Alert severity="error">{String(reprocessError.message)}</Alert>
            )}
            <Box>
              <Typography color="text.secondary">
                Status: {item.status} {item.message ? `— ${item.message}` : ''}
              </Typography>
              <Typography color="text.secondary">
              ...
              <Typography color="text.secondary">
                Totals — Parsed: ₦{fmt(parsedTotal)} | Lines: ₦
                {fmt(computedTotal)} | Diff: ₦{fmt(diff)}
              </Typography>
              <Stack direction="row" spacing={1} sx={{ mt: 1 }}>
                <Button
                  ...
                  onClick={async () => {
                    const q = String(
                      (item?.parsed as any)?.invoiceNumber ||
                        invoiceNumber ||
                        supplierName ||
                        '',
                    ).trim();
                    if (!q) {
                      notify(
                        'Enter or parse an invoice number first',
                        'warning',
                      );
                      return;
                    }
                    try {
                      const res = await searchPOs({
                        variables: { q },
                      ...
                      if (found) {
                        ...
                        notify(
                          'No purchase order found for that query',
                          'info',
                        );
                      }
                    } catch (e: any) {
              ...
              [
                {
                  ...
                  render: (r: any, i: number) =>
                    edit ? (
                      <TextField
                        size="small"
                        value={draftLines[i]?.description || ''}
                        onChange={(e) =>
                          setDraftLines((arr) => {
                            const next = [...arr];
                            next[i] = {
      ...
      <ApproveDialog
        ...
        onConfirm={async () => {
          const input: any = {
            ...
            createSupplierPayment: createPayment,
            receiveStock,
            receivedById: receivedById || null,
            confirmedById: confirmedById || null,
            useParsedTotal,
          };
          if (edit && draftLines?.length)
            input.overrideLines = draftLines.map((l: any) => ({
          ...
          // Ensure latest details (supplier, store, invoice #) are saved before approval
          try {
            await updateImport({
              variables: {
                input: {
                  ...
                  supplierName: supplierName || null,
                  storeId: storeId || null,
                  invoiceNumber: currentInvoiceNo || null,
                },
              },
            });
          } catch {}
          ...
          if (poId) {
            ...
            const q = String(
              (item?.parsed as any)?.invoiceNumber || supplierName || '',
            ).trim();
            if (q) {
              try {
                ...
                if (found)
                  setTimeout(() => navigate(`/purchase-orders/${found}`), 300);
              } catch {}
            }
          }
          setOpen(false);
        ...
        summary={{
          ...
          useParsedTotal,
          changedCount: (edit ? draftLines : []).filter(
            (l: any, i: number) =>
              edit && JSON.stringify(l) !== JSON.stringify(lines[i] || {}),
          ).length,
          changedDiffs: (edit ? draftLines : [])
            .map((l: any, i: number) =>
              edit && JSON.stringify(l) !== JSON.stringify(lines[i] || {})
                ? { index: i, before: lines[i] || {}, after: l }
                : null,
            )
            .filter(Boolean)
            .slice(0, 5) as any,
...
function ApproveDialog({
  ...
  summary: {
    ...
    changedCount?: number;
    changedDiffs?: Array<{ index: number; before: any; after: any }>;
  };
}) {
  const {
    parsedTotal,
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      ...
      <DialogContent>
        ...
        <Typography>
          Using: {useParsedTotal ? 'Parsed Total' : 'Lines Total'}
        </Typography>
        {Math.abs(diff) > 0.5 && (
          <Alert severity="warning" sx={{ mt: 1 }}>
            Totals differ by ₦{fmt(diff)}.
          </Alert>
        )}
      </DialogContent>
      <DialogActions>
