...
const SALES = gql`
  query ConsumerSalesByCustomer($customerId: String!, $take: Int) {
    consumerSalesByCustomer(customerId: $customerId, take: $take) {
      id
      status
      totalAmount
      createdAt
...
const RECEIPTS = gql`
  query ConsumerReceiptsByCustomer($customerId: String!, $take: Int) {
    consumerReceiptsByCustomer(customerId: $customerId, take: $take) {
      id
      issuedAt
      consumerSaleId
    }
...
export default function CustomerDetail() {
  ...
  const { data: sData, loading: sLoading, error: sError } = useQuery(SALES, { variables: { customerId: id, take: 20 }, skip: !id, fetchPolicy: 'cache-and-network' });
  const { data: rData, loading: rLoading, error: rError } = useQuery(RECEIPTS, { variables: { customerId: id, take: 20 }, skip: !id, fetchPolicy: 'cache-and-network' });
  const { data: storesData } = useQuery(STORES, { fetchPolicy: 'cache-first', errorPolicy: 'all' });
  const stores = storesData?.listStores ?? [];
  const sales = sData?.consumerSalesByCustomer ?? [];
  const receipts = rData?.consumerReceiptsByCustomer ?? [];
      ...
      <Grid container spacing={2}>
          ...
          <Card><CardContent>
            ...
            <TableList
              columns={React.useMemo(() => ([
                ...
                { key: 'totalAmount', label: 'Total', render: (s: any) => s.totalAmount },
              ] as any), [])}
              rows={sales}
              loading={sLoading}
        ...
        <Grid item xs={12} md={6}>
          <Card><CardContent>
            ...
            {rError && <Alert severity="error">{rError.message}</Alert>}
            <TableList
              columns={React.useMemo(() => ([
                { key: 'id', label: 'ID' },
