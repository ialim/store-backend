...
export class CatalogueDiagnosticsResolver {
  ...
  async variantsWithInvalidProducts(): Promise<OrphanVariantDiagnostic[]> {
    ...
    const ids = Array.from(new Set(variants.map((v) => v.productId).filter(Boolean))) as string[];
    const products = await this.prisma.product.findMany({ where: { id: { in: ids } }, select: { id: true } });
    const existing = new Set(products.map((p) => p.id));
    const invalid = variants.filter((v) => !v.productId || !existing.has(v.productId));
  ...
  async attachVariantToProduct(
    ...
    const prod = await this.prisma.product.findUnique({ where: { id: productId }, select: { id: true } });
    if (!prod) throw new Error('Target product not found');
    await this.prisma.productVariant.update({ where: { id: variantId }, data: { productId } });
    return true;
  }
}
