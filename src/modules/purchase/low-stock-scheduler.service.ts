...
import { PurchaseRequisitionStatus as PrismaPurchaseRequisitionStatus } from '@prisma/client';
...
export class LowStockSchedulerService {
  ...
  async handleInterval() {
    ...
    const byStore = new Map<
      string,
      Array<{ productVariantId: string; qty: number }>
    >();
    for (const s of lows) {
    ...
    for (const [storeId, items] of byStore.entries()) {
      ...
      this.logger.log(
        `Created low-stock requisition ${req.id} for store ${storeId}`,
      );
    }
  }
}
